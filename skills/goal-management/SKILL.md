...
1. 月次目標（全6ヶ月分）
   ...
   - マイルストーン
   - **重要**: 各月次目標に必ずIDを付与する
     - IDフォーマット: `[月-value-番号]`（例: [1月-hungry-1], [2月-owner-2]）
     - これにより週次目標との対応が明確になる

2. 週次目標（全約24週分）
   - 各月を4-5週に分解
   - 週ごとの具体的なタスク
   - **重要**: 各週次タスクに対応する月次目標IDを明記する
     - 例: `- [ ] [1月-hungry-1] Ruby学習Day 1を完了させる`
     - これにより、どの月次目標に対応しているか一目で分かる
...
6. **目標の定義**: 目標は「具体的な成果」を記載する。手段やツールの使用を目標にしない
   - 悪い例: 「〇〇を仕組み化する」「macloggerで記録する」
   - 良い例: 「Rubyの基礎・発展学習を完了する」（learning-supportは手段として記載）
7. **月次目標のID付与**: 月次目標には必ずIDを付与し、週次目標との対応を明確にする
   - IDフォーマット: `[月-value-番号]`（例: [1月-hungry-1], [2月-owner-2]）
